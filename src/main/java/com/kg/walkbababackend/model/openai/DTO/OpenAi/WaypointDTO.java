...
import com.kg.walkbababackend.model.openai.DB.ImageLink;
...
public record WaypointDTO(@JsonProperty("waypoint_name") String name, String description , List<String> imageLink){
    ...
    public WaypointDTO(WaypointInfo waypoint) {
        this(waypoint.getWaypointName(), waypoint.getWaypointDescription() , waypoint.getImageLink().stream()
                .map(ImageLink::getImageLink)
                .toList());
    }
}
